    ...
    public Iterable<? extends Processor<?>> getExtraProcessors()
    {
        return Lists.immutable.with(
                Processor.newProcessor(
                        ...
                        (Database srcDatabase, CompileContext context) ->
                        {
                            // Allow extensions to pre-process the Database protocol element before core compilation (only when imports present)
                            if (srcDatabase.importedIngests != null && !srcDatabase.importedIngests.isEmpty())
                            {
